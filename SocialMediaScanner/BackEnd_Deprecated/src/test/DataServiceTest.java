...
public class DataServiceTest {
	...
	for (int i = 0; i < jsonArray.length(); i++) {
		...
		assertTrue("content should contains " + keyword,
			Arrays.asList(arrWord).contains(keyword.toLowerCase()));
	    } catch (AssertionError e) {
	...
	try {
	    ...
	    assertEquals("if request is sent successfully, response code should be 200", responseCode, 200);
	} catch (AssertionError e) {
	...
	// now, we assume the only one user is Zingerman, so after
	// initialization, database should only contains
	// information related to Zingerman's, and contains nothing about
	// starbucks
	...
	// the response of searching zingerman's with keyword expensive should
	// be "valid";
	...
	// after pulling data for starbucks, then the search result should be
	// non-empty and valid;
	urlStr = "http://localhost:3456/search?company%20name=zingerman's%27s&keyword=expensive";
	response = makeHttpRequest(urlStr);
