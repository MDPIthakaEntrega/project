...
public class AccessData implements Data {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, JSONException {
		...
		ResponseStruct testresponse = new ResponseStruct(test_string, "Zingerman's", "Citygrid");
	...
	/*
	 * 
	 * Adds all the API's that have been implemented
	 * Called by this.init()
	 */
	private void initializeSources() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		for(File file: listofFiles) {
			...
			if(basename.equals("business") || basename.equals("Googleplaces") || basename.equals("ServiceInit")
					|| basename.equals("Twitter")){}
			else {
				sources.put(basename, Class.forName(this.getClass().getPackage().getName() + "." + basename));
			}
		}
	}
	 ...
	 */
	public void insertData(List<ResponseStruct> responses) throws InstantiationException, IllegalAccessException, JSONException, ClassNotFoundException {
		String api_name;
		for(ResponseStruct response: responses) {
			api_name = response.getAPIName();
			Class<?> api = Class.forName(this.getClass().getPackage().getName() + "." + api_name);
			try {
