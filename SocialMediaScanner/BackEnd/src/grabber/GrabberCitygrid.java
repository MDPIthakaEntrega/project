...
public class GrabberCitygrid extends DataGrabberGeneric {
	...
	private String generateURL(String companyName, String location,
			int pageNum, int rpp) throws UnsupportedEncodingException {
		...
		String pubParam = "publisher="
				+ URLEncoder.encode(publisherCode, "UTF-8");
		String formatParam = "format=" + URLEncoder.encode("json", "UTF-8");
		String rppParam = "rpp="
				+ URLEncoder.encode(String.valueOf(rpp), "UTF-8");
		String pageParam = "page="
				+ URLEncoder.encode(String.valueOf(pageNum), "UTF-8");

		String url = urlBase + whereParam + "&" + whatParam + "&" + pubParam
				+ "&" + formatParam + "&" + rppParam + "&" + pageParam;
