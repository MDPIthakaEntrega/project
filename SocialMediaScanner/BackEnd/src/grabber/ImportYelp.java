...
//		System.out.println(encoded);
		String requestURL = yelpURLNumPages + URLEncoder.encode(yelpBaseURL, "UTF-8") + URLEncoder.encode(yelpCompanyName, "UTF-8") + "&" + userParam + "&" + apiParam;
...
//			System.out.println(num_reviews_int);
		...
		for(int i = 0; i < numPages; i++){
			...
			requestURL = yelpURLResults + URLEncoder.encode(yelpBaseURL, "UTF-8") + URLEncoder.encode(yelpCompanyName, "UTF-8")
					+ yelpPageExtension + String.valueOf(i * RPP) + "&" + userParam + "&" + apiParam;
			try {
				response = sendGet(requestURL);
				System.out.println("response: " );
				System.out.println(response);
